...
The `shold` parameter controls whether the position loop remains active after a move reaches its target.

```text
set shold 0   # default, stop controller at target
set shold 1   # keep servo-position hold active at target
save
```
